...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ([['Admin ASISTA', 'admin@rumahpercaya.test', 'admin'], ['Keluarga Anindya', 'family@rumahpercaya.test', 'family'], ['Mitra Keluarga Agency', 'agency@rumahpercaya.test', 'agency']] as [$name,$email,$role]) {
            $u = User::firstOrCreate(['email' => $email], ['name' => $name, 'password' => 'DemoRumah123!']);
        ...
        foreach ($people as $i => [$name,$category,$city,$years,$rate,$unit,$skills]) {
            $u = User::firstOrCreate(['email' => 'worker'.($i + 1).'@rumahpercaya.test'], ['name' => $name, 'password' => 'DemoRumah123!']);
