...
@if($section !== 'workers')<div class="stats">@foreach(['workers'=>'Profil pekerja','agencies'=>'Partner agency','bookings'=>'Total booking','safety_reports'=>'Laporan privat'] as $key=>$label)<div class="panel"><span>{{ $label }}</span><strong>{{ $stats[$key] }}</strong><small>Data tersimpan di platform</small></div>@endforeach</div>@endif
...
@if($section !== 'workers' && $registrations->count())<section class="panel"><h2>Pendaftar belum melengkapi profil</h2>@foreach($registrations as $registrant)<p><strong>{{ $registrant->name }}</strong> · {{ $registrant->email }} · <a href="{{ route('admin.registrant', $registrant->id) }}">Lihat data & dokumen</a></p>@endforeach
...
@if($section === 'workers')@include('admin.workers-table')@else
...
@include('admin.pagination',['page'=>$rows,'anchor'=>''])</section>
@if($section === 'workers' && $registrations->count())<details class="panel worker-pending"><summary>Pendaftar belum melengkapi profil ({{ $registrations->total() }})</summary>@foreach($registrations as $registrant)<p><a href="{{ route('admin.registrant', $registrant->id) }}">{{ $registrant->name }} · {{ $registrant->email }}</a></p>@endforeach
@include('admin.pagination',['page'=>$registrations,'anchor'=>''])</details>@endif
<footer>Laporan keselamatan bersifat privat. Keputusan verifikasi dan pembayaran dicatat dalam audit trail.</footer></main>
@endsection
